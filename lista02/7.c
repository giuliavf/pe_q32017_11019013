...
float converte_temperatura(int in, int out, float n)
{
    int opc;
...
float converte_base(int in, int out, float n)
{
    int opc;
...
void conversor(int opc, int in, int out, float valor)
{
    ...
    switch (opc) {
        case 1:
            resp = converte_temperatura(in,out,valor);
        case 2:
            resp = converte_base(in,out,valor);
    }
    printf("%.2f ",resp);
